...
/*
    ...
    except for the following Message/patterns map
 */
var bytesToStringIgnorePatterns = {
    RpbGetResp: [
        /^vclock$/, // vlock will be converted to base64 in Client
        /^content\[\d+\]\.value$/ // don't convert content.value (including siblings)
    ]
};
...
RiakProtocol.define('Response', {
    read: function () {
        ...
        if (this.context.length <= 1) { // some response message dont have body, ex: RpbSetClientIdResp
            return null;
        }
