...
Pool.prototype._next = function () {
    var self = this, release;

    function _try() {
        return Promise.try(function () {
            var connection = self.connections.free.shift(), _resolve, server;

            if (connection === undefined) {
                if (self.connections.inuse.length < self.options.max) {
                    ...
                    return Promise.any(_.map(self.connections.inuse, 'promise')).then(_try);
                }
            }

            self.connections.inuse.push({
                id: connection.id,
            ...
            release = function (_connection) {
                _.remove(self.connections.inuse, { id: _connection.id });
