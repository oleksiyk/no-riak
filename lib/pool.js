...
Pool.prototype._next = function () {
    ...
    .disposer(function (connection, promise) {
        ...
        if (w !== undefined) {
            ...
            if ((self.connections.free.length >= self.options.min) && ((Date.now() - self.lastBursted) > self.options.idleTimeout)) { // release excessive connection
                self.options.release(connection);
            } else if (Date.now() > (connection.created + self.options.maxLifetime * (1 + Math.random() / 2))) { // release expired connection
                self.options.release(connection);
                if (self.connections.free.length < self.options.min) { // create replacement
                    connection = self.options.create();
                    if (connection) {
