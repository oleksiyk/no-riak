...
Connection.prototype.connect = function () {
    ...
    self.connecting = Promise.race([
        new Promise(function (resolve, reject) {
            setTimeout(function () {
                reject(new ConnectionError(self.server(), 'Connection timeout'));
        ...
        new Promise(function (resolve, reject) {
            ...
            self.socket.on('end', function () {
                self._disconnect(new ConnectionError(self.server(), 'Riak server has closed connection'));
            });
            self.socket.on('error', function (err) {
                var _err = new ConnectionError(self.server(), err.toString());
                reject(_err);
        ...
        if (self.options.auth) {
            return self.send('RpbStartTls').then(function () {
                return new Promise(function (resolve, reject) {
                    ...
                    self.socket.on('end', function () {
                        self._disconnect(new ConnectionError(self.server(), '[TLS] Riak server has closed connection'));
                    });

                    self.socket.on('error', function (err) {
                        var _err = new ConnectionError(self.server(), '[TLS] ' + err.toString());
                        reject(_err);
                        self._disconnect(_err);
                    });
...
Connection.prototype.close = function () {
    this._disconnect(new ConnectionError(this.server(), 'Connection closed'));
};
