...
Client.prototype._releaseConnection = function (err, c) {
    if (err) { // connection failed with ConnectionError
        this.servers.updateWeight({ host: c.options.host, port: c.options.port }, 0);
        this._ping(c.options.host, c.options.port);
    }
    c.close();
};
...
Client.prototype._ping = function (host, port) {
    ...
    return (function _try() {
        return connection.send('RpbPingReq').catch(function () {
    ...
    .then(function () {
        ...
        return Promise.delay(20000).then(function () { // give it some time to start vnodes
