 ...
 */
Client.prototype.fetchPut = function (params) {
    var self = this;
...
/**
 * Experimental. Fetches head of the object to get its vclock before performing del request.
 * Both requests are sent on the same connection.
 *
 * @param  {Object} params
 * @return {Promise}        [description]
 */
Client.prototype.fetchDel = function (params) {
    var self = this;

    return self.pool.reduce('RpbGetReq', 'RpbDelReq', function (prev, request, index) {
        if (index === 0) {
            return {
                bucket: params.bucket,
                key: params.key,
...
Client.prototype.getBucketType = function (type) {
    var self = this;
    return self.pool.send('RpbGetBucketTypeReq', type);
};
