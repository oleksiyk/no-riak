...
function Client(options) {
    ...
    self.failoverServers = new WRRPool();
    ...
    if (self.options.retries === undefined) {
        self.options.retries = self.servers.size() * self.options.maxConnectionErrors;
    }
...
Client.prototype._createConnection = function () {
    ...
    if (server === null) {
        server = this.failoverServers.next();
        if (server === null) {
            return null;
        }
        if (!this.usingFailover) {
            this.emit('net:failover:on');
        }
...
Client.prototype._checkConnection = function (err, c) {
    ...
    if (err instanceof errors.RiakConnectionError) {
        ...
        if (s.value.errors.length >= self.options.maxConnectionErrors) {
            servers.updateWeight({ host: c.host(), port: c.port() }, 0);
            self._ping(c.host(), c.port(), c.isFailover());
    ...
    if (c.isFailover() && self.usingFailover === false) { // primary cluster is up, close failover connections
        c.close();
        return self._createConnection();
    }
...
Client.prototype.useFailover = function () {
    this.pool.close('all');
    this.servers = this.failoverServers;
    this.usingFailover = true;
};
...
Client.prototype._ping = function (host, port, isFailover) {
    ...
    .then(function () { // host is back
        var servers = isFailover ? self.failoverServers : self.servers;
        var p = servers.get({ host: host, port: port });

        p.value.errors = [];
        self._pinging[host + ':' + port] = false;
        connection.close();
        servers.updateWeight({ host: host, port: port }, p.value.origWeight);

        if (!isFailover && self.usingFailover) { // at least 1 primary server is up, now close failover connection(s)
            self.pool.close('free', function (_c) {
                return _c.isFailover();
            });
            self.emit('net:failover:off');
            self.usingFailover = false;
        }
...
Client.prototype._request = function (name, params) {
    ...
    // _.set(self.stats, [name, _.get(params, 'bucket', 'n/a')], _.get(self.stats, [name, _.get(params, 'bucket', 'n/a')], 0) + 1);
...
Client.prototype.resetStats = function () {
    var self = this;
